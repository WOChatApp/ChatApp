...
import {
  ...
  deleteChatMessage,
  updateMessage,
} from "../state/actions/chat.actions";
import {
  ImagePickerOptions,
  Mediafilepicker,
  VideoPickerOptions,
...
export class ChatComponent {
  ...
  user: any;
  users: any;
  @ViewChild("myScrollView", { static: false })
  myScrollView: ElementRef<ScrollView>;
  ...
  async loadChatMessages() {
    try {
      ...
      this.store.select(selectChatMessages).subscribe((messages: any) => {
  ...
  async sendMessage() {
    ...
    if (this.isEdited) {
      ...
      this.store.dispatch(
        updateMessage({ channelId, messageID, updatedMessageObject })
      );
      this.isEdited = false;
